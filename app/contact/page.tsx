...
export default function ContactPage() {
  ...
  return (
    <>
      ...
      <section className="pb-28 px-6" style={{ background: "#FAF6F0" }}>
          ...
          <div className="md:col-span-2 space-y-8">
            {[
              ...
              },
              {
                ...
                sub: "Nepal 44600 — full address on booking",
                color: "#6B2D8B",
              ...
              <a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer"
                ...
                <div>
                  ...
                  <p className="text-xs font-light mt-0.5" style={{ color: "rgba(42,18,8,0.35)" }}>{item.sub}</p>
                </div>
              </a>
            ))}
          </div>
          ...
          <div className="md:col-span-3">
            {submitted ? (
              <div className="card-light p-12 text-center h-full flex flex-col items-center justify-center">
                ...
                <p className="text-sm font-light" style={{ color: "rgba(42,18,8,0.45)" }}>
              ...
              <form onSubmit={handleSubmit} className="card-light p-8 space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-xs tracking-widest uppercase font-light mb-2" style={{ color: "rgba(42,18,8,0.4)" }}>
                      Your Name
                    </label>
                    <input
                      required
                ...
                <div>
                  ...
                  <select
                    ...
                    style={{
                      background: "rgba(255,255,255,0.04)",
                      border: "1px solid rgba(255,255,255,0.08)",
                      color: form.program ? "#FAF0DC" : "rgba(42,18,8,0.3)",
                    }}
                  >
                    <option value="" style={{ background: "#E8DDD4" }}>Select a program…</option>
                    {programs.map(p => (
                      <option key={p} value={p} style={{ background: "#E8DDD4" }}>{p}</option>
                    ))}
                  </select>
                </div>
                ...
                <button type="submit"
                  className="cta-lift w-full py-3.5 rounded-full font-medium text-sm"
                  style={{ background: "#F7941D", color: "#FAF6F0" }}>
                  Send Message
                </button>

                <p className="text-xs text-center font-light" style={{ color: "rgba(42,18,8,0.25)" }}>
                  Prefer instant reply? Message us directly on{" "}
                  <a href="https://wa.me/9779800000000" className="underline" style={{ color: "#8DC63F" }}>WhatsApp</a>
                </p>
              </form>
            )}
          </div>
