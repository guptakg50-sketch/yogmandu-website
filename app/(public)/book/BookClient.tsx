...
function BookPageInner({ teachers, classPhotos }: { teachers: TeacherPhoto[]; classPhotos: ClassPhoto[] }) {
  ...
  // Match the class/teacher named in the ?cls= link to the photos the admin has
  // uploaded. Names are compared case-insensitively and trimmed because they
  // travel through the URL.
  const sameName = (a: string, b: string) => a.trim().toLowerCase() === b.trim().toLowerCase();
  const teacherPhoto = parsedClass?.instructor
    ? teachers.find((t) => sameName(t.name, parsedClass.instructor))?.photo ?? ""
    : "";
  const classImage = parsedClass?.name
    ...
    <>
      ...
      {parsedClass && step !== 3 && (
        <div style={{
          ...
          {/* The class photo if the session has one, otherwise the teacher's
              portrait — both are managed in the admin. Falls back to the icon
              when neither has been uploaded yet. */}
          {classImage ? (
            <img src={classImage} alt="" width={64} height={64}
              style={{ width: 64, height: 64, borderRadius: 12, objectFit: "cover", flexShrink: 0,
                border: "2px solid rgba(255,255,255,0.9)", boxShadow: "0 4px 14px rgba(107,45,139,0.18)" }} />
          ) : teacherPhoto ? (
            <img src={teacherPhoto} alt={parsedClass.instructor} width={64} height={64}
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ margin: 0, fontSize: "0.95rem", color: "#7A5840" }}>
              {parsedClass.day} · {parsedClass.time}
            </p>
            {parsedClass.instructor && (
              <p style={{ margin: "4px 0 0", display: "flex", alignItems: "center", gap: 7,
                fontSize: "0.9rem", color: "#4A2E1A", fontWeight: 500 }}>
                {/* Show the teacher too when the thumbnail is the class photo */}
                {classImage && teacherPhoto && (
                  <img src={teacherPhoto} alt="" width={24} height={24}
                    style={{ width: 24, height: 24, borderRadius: "50%", objectFit: "cover" }} />
                )}
                with {parsedClass.instructor}
              </p>
            )}
          </div>
        </div>
