...
export default async function SoundHealingPage() {
  const [hub, inSession] = await Promise.all([
    getHubConfig("SOUND_HEALING"),
    getSectionContent("SOUND_IN_SESSION"),
  ]);
  return (
    <>
      ...
      <section className="py-24 px-6" style={{ background: "#F9F5FF" }}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-xs tracking-[0.3em] uppercase mb-4 font-medium" style={{ color: "#6B2D8B" }}>{inSession.eyebrow}</p>
            <h2 className="text-4xl md:text-5xl font-light" style={{ fontFamily: "Cormorant Garamond, serif", color: "#2A1208" }}>
              {inSession.title} <em style={{ color: "#A65808" }}>{inSession.titleEm}</em>
            </h2>
